...
pub trait Pass<'ast, P: Pointer>: Sized {
    ...
    fn visit_func_attr(&mut self, _node: &'ast FuncAttr<P>) {
        // todo walk, when its more than just an identifier
        self.nop()
    }
...
pub fn walk_top_level_decl<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast TopLevelDecl<Ptr>,
) {
    match node.kind {
...
pub fn walk_func_decl<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast FuncDecl<Ptr>,
) {
...
pub fn walk_infix_def<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast InfixDef<Ptr>,
) {
...
pub fn walk_infix_func_call<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast InfixFuncCall<Ptr>,
) {
    v.visit_ident(&node.ident);
...
pub fn walk_infix_op_call<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast InfixOpCall<Ptr>,
) {
    v.visit_op(&node.op);
    v.visit_expr(&node.lhs);
    v.visit_expr(&node.rhs);
...
pub fn walk_func_call<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast FuncCall<Ptr>,
) {
    v.visit_expr(&node.lhs);
    for arg in &node.args {
        v.visit_expr(arg);
...
pub fn walk_func_type<'ast, Ptr: Pointer, P: Pass<'ast, Ptr>>(
    v: &mut P,
    node: &'ast FuncType<Ptr>,
) {
